    ...
    <div class="m-5 mb-1">
      ...
      <div class="card mb-10">
        <div class="d-flex align-items-center">
          <div class="card-body">
            <label class="fs-6 form-label fw-bold text-gray-900 me-3"
              >검색</label
            >
            <div class="d-flex align-items-center">
              ...
              <div>
                <button class="btn btn-primary px-4" @click="searchEnter">
                  검색
                </button>
              </div>
            </div>
      ...
      <div class="m-5">
        <ul class="nav nav-tabs nav-line-tabs mb-5 fs-6">
          <li v-for="(item, index) in tabList" :key="index" class="nav-item">
            <a
              href="#"
              :class="['nav-link', { active: selectedTab === item }]"
      ...
      <div class="mt-5">
        <div class="d-flex flex-wrap flex-stack">
          ...
          <div class="d-flex flex-wrap align-items-center my-1">
            <h3 class="fw-bold me-5 my-1 ms-5">
          ...
          <tr v-for="(item, index) in paginatedCards" :key="index">
            <td>
              <div class="card card-flush shadow-sm m-5">
                <div
                  class="card-side d-flex justify-content-between m-7 align-items-center"
                >
...
onMounted(async () => {
  try {
    // 백엔드 API에서 카드 리스트를 불러옴
    const response = await axios.get('/api/product/card');
    cards.value = response.data; // API에서 받은 데이터를 cards에 저장
  } catch (error) {
...
const filteredCards = computed(() => {
  return cards.value.filter((item) => {
    const matchesSearch =
      item.card_name.includes(searchCard.value) ||
      item.corp_name.includes(searchCard.value);
    const matchesCategory =
      selectedTab.value === '전체' || item.category === selectedTab.value;
